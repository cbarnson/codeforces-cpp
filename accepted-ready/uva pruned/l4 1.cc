...
bool isPercent(string s) { return (s.back() == '%'); }

bool isCal(string s) { return (s.back() == 'C'); }
...
int getCal(string s) { return stoi(string(s.begin(), s.end() - 1)); }

string toPercent(double percent) { return to_string(percent) + "%"; }
...
void changeToPercent(vector<string> &t) {
    ...
    for (int i = 0; i < t.size(); i++) {
        if (isCal(t[i])) {
            // change
            double newPercent = static_cast<double>(getCal(t[i])) *
                                static_cast<double>(per2) /
                                static_cast<double>(cTotal);
