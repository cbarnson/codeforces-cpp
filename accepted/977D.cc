...
int main() {
  ...
  // 1 <= a[i] <= 10^18, for each i in [0, n-1], so need long long, which gives
  // room for integers up to (2^63)-1, which leaves us plenty of room (even if
  // we're careless doing our checks; since, at worst, 3 * 10^18 < (2^63)-1)
  vector<ll> a(n);
  FR(i, n) { cin >> a[i]; }

  // Adjacency list representation of the possible paths from any number to any
  // other number.
