...
int main() {
    ...
    while (t--) {
        ...
        // smallest period is less than largest possible seed values, so makes
        // sense to mod our seed values here
        f[0] = a % mod[m], f[1] = b % mod[m];

        // compute for the rest of our period, taking the last mod[m] digits
        // only
        for (int i = 2; i < p[m]; i++) {
